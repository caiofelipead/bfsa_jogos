...
def compute_metrics(df_jogos, df_obs):
    ...
    if "ID_Jogo" in df_jogos.columns:
        ...
        df_merged["Jogadores_Cadastrados"] = df_merged["Jogadores_Cadastrados"].fillna(0).astype(int)
    else:
...
def main():
    ...
    with tab_overview:
        ...
        st.plotly_chart(plot_timeline(df_merged), use_container_width=True)
    ...
    with tab_analistas:
        if not df_analistas.empty:
            ...
            with col1:
                ...
                st.plotly_chart(plot_analistas_bar(df_analistas), use_container_width=True)
            with col2:
                section_title("🕸️", "PERFIL RADAR")
                st.plotly_chart(plot_analistas_radar(df_analistas), use_container_width=True)
    ...
    with tab_camps:
        if not df_camps.empty:
            ...
            with col1:
                ...
                st.plotly_chart(plot_campeonatos_bar(df_camps), use_container_width=True)
            with col2:
                section_title("🎯", "MÉDIA POR JOGO")
                st.plotly_chart(plot_media_campeonato(df_camps), use_container_width=True)
